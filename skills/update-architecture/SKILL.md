...
- **Sequence invocation labels**: label `->>` arrows with the call-site binding and method — `{receiverBinding}.{method}` — not the Mermaid participant alias and not a prose summary. Good: `Gateway->>SystemRepo: systemRepo.getActiveGenerationId`. Bad: `getActiveGenerationId()`, `SystemRepo.getActiveGenerationId`, `acquire active generationId`. Strip `this.` / `props.` from `{var}`. Omit `()` unless args distinguish overloads. Family diagrams may keep wildcards such as `gatewayApi.get*FrontendApi`. Return arrows (`-->>`) keep result payloads; user/process steps and unnamed inline checks stay short predicates.
